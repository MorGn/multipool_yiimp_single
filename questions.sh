...
RESULT=$(dialog --stdout --default-item 1 --title "Server IP these automatically detected" --menu "Choose one" -4 60 6 \
 \
1 "Sets server IP automatically public IP" \
 \
2 "Sets server IP automatically private IP" \
 \
3 "Manual IP input" \
)
if [ $RESULT = ]
...
then
clear;
DomainName=$(get_publicip_from_web_service 4)
StratumURL=$(get_publicip_from_web_service 4)
UsingSubDomain=no
fi
...
then
clear;
DomainName=$(get_default_privateip 4)
StratumURL=$(get_default_privateip 4)
UsingSubDomain=no
fi
...
then
clear;
DEFAULT_DomainName=192.168.0.1
input_box "Manual IP input" \
"" \
${DEFAULT_DomainName} \
DomainName
UsingSubDomain=no
...
input_box "Manual stratum IP input" \
"" \
${DEFAULT_StratumURL} \
StratumURL
